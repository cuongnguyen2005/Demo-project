...
class FinancesBloc extends Bloc<FinancesEvent, FinancesState> {
  ...
  FinancesBloc() : super(FinancesInitial()) {
    on<FinancesEvent>((event, emit) {});
    //get cate expense
    on<FinancesGetCateEvent>((event, emit) async {
      List<Category> cates = [
        Category(
            id: "",
            cateName: "Khoản thu",
      ...
      List<Category> curExpenseList = [];
      List<Category> curIncomeList = [];
      for (var element in cates) {
        if (element.cateID == 2) {
          curExpenseList.add(element);
        }
        if (element.cateID == 1) {
      ...
      emit(FinancesState(
          cateExpense: curExpenseList, cateIncome: curIncomeList));
    });

    //add and edit expense
    on<FinancesAddAndEditExpenseEvent>((event, emit) async {
    ...
    //add and edit expense
    on<FinancesAddAndEditIncomeEvent>((event, emit) async {
      if (moneyController.text.isNotEmpty) {
        if (nameCate != '') {
          //hiển thị thông báo thành công
          showDialog(
